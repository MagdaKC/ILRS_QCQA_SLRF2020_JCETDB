...
{$conn_string = "host=jcetdb.rs.umbc.edu port=5432 dbname=ilrsa_d_slrf2020 user=jasongroup  password=Ichinen  connect_timeout=2";}
if ($_POST[combination_center]=="ilrsb")
{$conn_string = "host=jcetdb.rs.umbc.edu port=5432 dbname=ilrsb_d_slrf2020 user=jasongroup  password=Ichinen  connect_timeout=2";}
...
$query1 = "SELECT EXTRACT(EPOCH from observation_date at time zone 'utc-00') *1000 as datetime1, $plota as value1, $splota as value2 from $tab where observation_date > '$start_date' and observation_date < '$end_date' order BY observation_date ;";}
if ($_POST[combination_center]=="ilrsb")
{$conn_string = "host=jcetdb.rs.umbc.edu port=5432 dbname=ilrsb_d_slrf2020 user=jasongroup  password=Ichinen  connect_timeout=2";}
//$query1 = "SELECT EXTRACT(EPOCH from observation_date at time zone 'utc-00') *1000 as datetime1, $plota as value1, $splota as value2 from $tab where observation_date > '$start_date' and observation_date < '$end_date' and $plota  < $limita1 and $plota > $limitb1  order BY observation_date ;";
$query1 = "SELECT EXTRACT(EPOCH from observation_date at time zone 'utc-00') *1000 as datetime1, $plota as value1, $splota as value2 from $tab where observation_date > '$start_date' and observation_date < '$end_date' order BY observation_date ;";
...
$query1 = "SELECT EXTRACT(EPOCH from observation_date at time zone 'utc-00') *1000 as datetime1, $plota as value1 from $tab where observation_date > '$start_date' and observation_date < '$end_date'  order BY observation_date ;"; }
if ($_POST[combination_center]=="ilrsb")
{$conn_string = "host=jcetdb.rs.umbc.edu port=5432 dbname=ilrsb_d_slrf2020 user=jasongroup  password=Ichinen  connect_timeout=2";
...
if ($_POST[combination_center]=="ilrsb")
{if($_POST[PLOTS2]=="GRMS" or $_POST[PLOTS2]=="CRMS")
{$queryS = "SELECT distinct AVG ((|/(($plota*$plota)/3))*1000)  from $tab where  observation_date > '$start_date' and observation_date < '$end_date'  and (|/(($plota*$plota)/3))*1000 < $limita1 and (|/(($plota*$plota)/3))*1000 > $limitb1 ;";}
else
{$queryS = "SELECT distinct AVG ($plota)  from $tab where  observation_date > '$start_date' and observation_date < '$end_date'  and $plota  < $limita1 and $plota > $limitb1 ;";}}
...
if ($_POST[combination_center]=="ilrsb")
{if($_POST[PLOTS2]=="GRMS" or $_POST[PLOTS2]=="CRMS")
{$queryS = "SELECT count ((|/(($plota*$plota)/3))*1000)  from $tab where  observation_date > '$start_date' and observation_date < '$end_date'  and (|/(($plota*$plota)/3))*1000 < $limita1 and (|/(($plota*$plota)/3))*1000 > $limitb1 ;";}
else
{$queryS = "SELECT count ($plota)  from $tab where  observation_date > '$start_date' and observation_date < '$end_date'  and $plota  < $limita1 and $plota > $limitb1 ;";}}
...
if ($_POST[combination_center]=="ilrsb")
{if($_POST[PLOTS2]=="GRMS" or $_POST[PLOTS2]=="CRMS")
{$queryS = "SELECT distinct STDDEV ((|/(($plota*$plota)/3))*1000)  from $tab where  observation_date > '$start_date' and observation_date < '$end_date'  and (|/(($plota*$plota)/3))*1000< $limita1 and (|/(($plota*$plota)/3))*1000 > $limitb1 ;";}
else
{$queryS = "SELECT distinct STDDEV ($plota)  from $tab where  observation_date > '$start_date' and observation_date < '$end_date'  and $plota  < $limita1 and $plota > $limitb1 ;";}}
...
<script>
    ...
    position: [20, 35],
    resizable: false,width: 320,height: 80});
...
//if ($_POST[combination_center]=="ilrsb")
//{if($_POST[PLOTS2]=="GRMS" or $_POST[PLOTS2]=="CRMS" or $_POST[PLOTS2]=="Scale")
// {$query2 = "SELECT observation_date as datetime2, to_char((|/(($plota*$plota)/3))*1000, '999999D99') as value from $tab where observation_date > '$start_date' and observation_date < '$end_date' ORDER BY observation_date ;"; }}
//if ($_POST[combination_center]=="ilrsa")
//{if($_POST[PLOTS2]=="GRMS" or $_POST[PLOTS2]=="CRMS" or $_POST[PLOTS2]=="Scale")
//{ $query2 = "SELECT observation_date as datetime2, to_char($plota, '999999D99') as value from $tab where observation_date > '$start_date' and observation_date < '$end_date' ORDER BY observation_date ;"; }}
//
//echo $query2.'<br/><br/>';
$resultF = pg_query($query2) or die("" );
$data = array();
$output= $tt. "\n";
fwrite($file, $output);
$output ="  DATE     ," ;
...
fwrite($file, $output); 
while ($row = pg_fetch_array($resultF, null, PGSQL_ASSOC)) {
//echo $datetime;
//echo $row['value'];
