...
{$y_max="2";}
if ($_POST[PLOTS2]=="vf" or $_POST[PLOTS2]=="vce")
//{$y_max="0.0005";}
{$y_max="null";}
...
elseif ( $_POST[PLOTS2]=="vce" )
{ $query = "SELECT EXTRACT(EPOCH from observation_date at time zone 'utc-00') *1000 as datetime1, $plot as value from $tab where observation_date > '$start_date' and observation_date < '$end_date' and $plot <> 0 ORDER BY observation_date  ;";}
else
...
{$queryS = "SELECT distinct AVG ($plot)  from $tab where  observation_date > '$start_date' and observation_date < '$end_date'and $plot <>0 ;";}
else
...
elseif ( $_POST[PLOTS2]=="vce" )
{$queryS = "SELECT distinct STDDEV ($plot) from $tab where observation_date > '$start_date' and observation_date < '$end_date' and $plot <>0 ;";}
else
...
elseif ( $_POST[PLOTS2]=="vce" )
{$queryS = "SELECT count ($plot) from $tab where observation_date > '$start_date' and observation_date < '$end_date' and $plot<>0 ;";}
else
...
series : [{<?php echo $REG11 ?> <?php echo $REG22 ?> <?php echo $REG33 ?> <?php echo $REG44 ?> <?php echo $REG55 ?> lineWidth : 0,showInLegend: true,   name: '<?php echo $type ?>  <?php echo $station_name ?>' ,
...
elseif ( $_POST[PLOTS2]=="vce" )
{ $query2 = "SELECT observation_date as datetime2, to_char($plot, '999999D') as value from $tab where observation_date > '$start_date' and observation_date < '$end_date' and $plot <>0 ORDER BY observation_date ;"; }
else
