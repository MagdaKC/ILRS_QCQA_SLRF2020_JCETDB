...
extend(Chart.prototype, {
	...
	getSVG: function (additionalOptions) {
		...
		svg = chart.container.innerHTML; // PERSO : why make a copy ?
		
		var test = svg.substr(0,4);
		if(test != '<svg' && test != '<SVG'){
			// cas internet explorer :
			return chart.getSVGByCopy(additionalOptions);
		}
		...
		svg = svg
			...
			var index = svg.indexOf('NaN'); // si pr̩sence de NaN alors la conversion echoue
			if(index>=0)
				svg = svg.substr(0,index).concat(svg.substr(index+3));
	...
	getSVGByCopy: function (additionalOptions) {
		var chart = this,
			chartCopy,
			sandbox,
			svg,
